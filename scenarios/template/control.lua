...
if script.mod_name == "level" and script.active_mods[MOD_NAME] then return end -- Don't init twice. Mod first.
...
local modules = {}
modules.multi_template_mod = require("multi-template-mod")
...
if remote.interfaces["disable-" .. script.mod_name] then
	...
	if modules.better_commands then
		if modules.better_commands.handle_custom_commands then
			modules.better_commands.handle_custom_commands(modules.multi_template_mod) -- adds commands
		end
